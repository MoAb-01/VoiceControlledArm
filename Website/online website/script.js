...
function SetupStream(stream) {
  ...
  recorder.onstop = async e => {
    ...
    if (chosenLanguage == "English") {
      ...
      try {
  const response = await fetch("http://192.168.1.230:5000/command", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify({ command: lemurCommand })
  });

  if (response.ok) {
    console.log(`✅ [DEBUG] Command "${lemurCommand}" successfully sent to Raspberry Pi`);
  } else {
    console.warn(`⚠️ [DEBUG] Raspberry Pi responded with status ${response.status}`);
  }

} catch (error) {
  console.error("❌ [DEBUG] Failed to send command to Raspberry Pi:", error);
}
  ...
  try {
    const response = await fetch("http://localhost:3000/lemur", {
      ...
      body: JSON.stringify({   // ← body at same level as headers
        transcriptiontext: transcriptionText,  // ✅ not input_text
